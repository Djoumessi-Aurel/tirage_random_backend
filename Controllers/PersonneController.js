...
const store = async (req, res, next) => {
    try {
        let personne = new Personne({
            ...
            a_tire: req.body.a_tire?.toLowerCase(),
...
const tirerPersonne = async (req, res, next) => {
    try {
        let nomX = req.body.nom?.trim()?.toLowerCase()
        let persX = await Personne.findOne({nom: nomX})
        if(!persX) {res.status(400).json({message:"Erreur. Vous n'êtes pas concerné par ce tirage."}); return;}
        if(persX.a_tire) {res.status(200).json({message:`Hello ${persX.nom}. Votre binôme est ${persX.a_tire}`}); return;}
        ...
        await persX.save()
        res.status(200).json({message:`Hello ${persX.nom}. Votre binôme est ${persX.a_tire}`})
    } catch (error) {
        res.status(400).json({
            message:'Une erreur est survenue!', content: error.message
        })
